...
shieldips['10.2.71.1']["current_playback_transission_count"] = 1000
shieldips['10.2.71.2']["current_playback_transission_count"] = 1000
shieldips['10.2.71.3']["current_playback_transission_count"] = 1000
...
"Plex" : {
       ...
       "videoplayback_required_activity" : "VideoPlayerActivity",
                  "videoplayback_detect" : True,
      "videoplayback_required_wakelocks" : 2,
"videoplayback_transission_count_needed" : 0,
},

"You Tube" : {
                                   "app" : "com.google.android.youtube.tv",
       "videoplayback_required_activity" : "MainActivity",
                  "videoplayback_detect" : True,
      "videoplayback_required_wakelocks" : 2,
"videoplayback_transission_count_needed" : 0,
},
...
"Amazon Prime" : {
       ...
       "videoplayback_required_activity" : "IgnitionActivity",
                  "videoplayback_detect" : True,
      "videoplayback_required_wakelocks" : 1,
"videoplayback_transission_count_needed" : 0,
},
...
def sendbutton(myIP, myKeycode):
   ...
   # print str(adb_keycodes[myKeycode])
...
def check_online(ip):
    ...
    for row in devices.split('\n'):
       if ip in row and ( "online" in row or "device" in row):
...
def check_online_and_connect(ip):
    ...
    #print " --> Device " + ip + " is still offline. Restarting adb server"
...
#
    print
    print "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    print " --> Device " + ip + " is still offline. Something is broken"
    print "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    print
                ...
                if apps[myApp]["videoplayback_detect"]:
                    ...
                    if myWakeLocks >= apps[myApp]["videoplayback_required_wakelocks"]:
                        ...
                        if apps[myApp]["videoplayback_required_activity"].lower() in focuswindow.lower():
                            ...
                            mVideoPlayback = "on"
                            #apps[myApp]["current_playback_transission_count"] = 0
                            shieldips[ip]["current_playback_transission_count"] = 0
                            
                    if mVideoPlayback == "off":
                        print "No Videoplayback detected"

                        #apps[myApp]["current_playback_transission_count"] += 1
                        shieldips[ip]["current_playback_transission_count"] += 1
                        

                        if not shieldips[ip]["current_playback_transission_count"] >= apps[myApp]["videoplayback_transission_count_needed"] :
                            print "Current Transition count : " + str(shieldips[ip]["current_playback_transission_count"]) + " of needed : " + str(apps[myApp]["videoplayback_transission_count_needed"])
                            print "NOT changing videoplayback state YET ..."
                            mVideoPlayback = "on"
